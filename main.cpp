...
int main()
{
	...
	Auto car;
	int answer;
	cout << " Приехала на автомойку машина? (Введите 1 - если да, 0 - если нет)" << endl;
	cin >> answer;
	do
	{
		if (answer == 1)
		{
			...
			if (cars.size() == 1)
			{
				cout << " В очереди стоит " << cars.size() << " машина" << endl;
			}

			if (cars.size() > 1 && cars.size() < 5)
			{
				cout << " В очереди стоят " << cars.size() << " машины" << endl;
			}

			if (cars.size() > 5)
			{
				cout << " В очереди стоят " << cars.size() << " машин" << endl;
			}
